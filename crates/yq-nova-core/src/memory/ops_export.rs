...
pub async fn export_memories(svc: &MemoryService, input: ExportInput) -> NovaResult<ExportOutput> {
    ...
    Ok(ExportOutput { count, offset: input.offset, items })
}
...
mod tests {
    ...
    async fn export_all_memories() {
        ...
        let a = svc.remember(RememberInput { content: "memory one", importance: 0.5, ..Default::default() }).await.unwrap();
        let b = svc.remember(RememberInput { content: "memory two", importance: 0.8, ..Default::default() }).await.unwrap();
    ...
    async fn export_filter_by_source() {
        let svc = temp_svc().await;
        let _user = svc.remember(RememberInput { content: "user note", source: MemorySource::User, importance: 0.5, ..Default::default() }).await.unwrap();
        let _agent = svc.remember(RememberInput { content: "agent thought", source: MemorySource::Agent, importance: 0.5, ..Default::default() }).await.unwrap();

        let filter = MemoryFilter { source_in: Some(vec![MemorySource::User]), ..Default::default() };
        let out = export_memories(&svc, ExportInput { filter: Some(filter), ..Default::default() }).await.unwrap();
    ...
    async fn export_pagination() {
        let svc = temp_svc().await;
        for i in 0..5 {
            svc.remember(RememberInput { content: &format!("memory-{i}"), importance: 0.1, ..Default::default() }).await.unwrap();
        }

        let page1 = export_memories(&svc, ExportInput { limit: 2, offset: 0, ..Default::default() }).await.unwrap();
        assert_eq!(page1.count, 2);
        assert_eq!(page1.offset, 0);

        let page2 = export_memories(&svc, ExportInput { limit: 2, offset: 2, ..Default::default() }).await.unwrap();
        assert_eq!(page2.count, 2);
        assert_eq!(page2.offset, 2);

        let page3 = export_memories(&svc, ExportInput { limit: 2, offset: 4, ..Default::default() }).await.unwrap();
